      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <form method="post" id="NewGroup">
            ...
            <div class="form-group bdr">
              <h5>Weakness</h5>
              <textarea type="text" class="form-control" id="weakness" maxlength="200"></textarea>
              <span id="err_weakness" style="color:red"></span>
            </div>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $('#example').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
        'url': '<?php echo base_url() ?>Interview/getOldInterviewMenu'
      },
      'columns': [{
          data: 'id'
        },
  ...
  function startInterview(feedbackId) {
    swal({
        title: "Are you sure?",
        text: "You want to edit Interview",
        type: "warning",
        showCancelButton: true,
        confirmButtonColor: "#DD6B55",
        confirmButtonText: "Yes",
      ...
      function(isConfirm) {
        if (isConfirm) {
          swal.close();
          console.log('eveny_id', feedbackId);
          $.ajax({
            url: '<?php echo base_url(); ?>Interview/getfeedback',
            type: 'POST',
            dataType: "json",
            data: {
            ...
            success: function(res) {
              ...
              $("#profile").val(response.profileSkills);
              $("#profile_result b").html(response.profileSkills);
              $("#behaviour").val(response.behaviour);
              $("#behaviour_result b").html(response.behaviour);
              $("#team").val(response.teamPlayer);
              $("#team_result b").html(response.teamPlayer);
              $("#communication").val(response.communication);
              $("#communication_result b").html(response.communication);
              $("#attitude").val(response.attiToWordPostion);
              $("#attitude_result b").html(response.attiToWordPostion);
              $("#work").val(response.workExperience);
              $("#work_result b").html(response.workExperience);
              $("#technicalSkills").val(response.technicalSkills ? response.technicalSkills : '0');
              $("#technicalSkills_result b").html(response.technicalSkills ? response.technicalSkills : '0');
              $("#logicalSkills").val(response.logicalSkills ? response.logicalSkills : 0);
              $("#logicalSkills_result b").html(response.logicalSkills ? response.logicalSkills : 0);
              $("#strength").val(response.strength);
              $("#weakness").val(response.weekness);
              $("#additionalComment").val(response.additionalComment);
              $("#timetaken").val(response.totalTime);
    ...
    swal({
      ...
      function(isConfirm) {
        if (isConfirm) {
          ...
          $.ajax({
            url: '<?php echo base_url(); ?>Interview/submitInterview2',
            type: 'POST',
            dataType: "json",
            data: postdata,
