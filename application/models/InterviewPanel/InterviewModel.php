...
class InterviewModel extends CI_Model
{
    ...
    public function submitInterview2($postdata)
    {
        $id = $postdata['feedbackId'];
        unset($postdata['feedbackId']);
        $this->db->where('id',  $id);
        $this->db->update('tblFeedback', $postdata);
    ...
    function getOldInterviewMenu($postData)
    {
        ...
        THEN 'Strongly Hire'  END ) from tblFeedback where roundId=t2.id
        and resumeId=t1.id and createdBy=t2.interviewerId) AS recommendation ,
        (select id from tblFeedback where roundId=t2.id) as feedbackId,
        (SELECT  (CASE WHEN DATE_ADD(createdDate,INTERVAL 2 HOUR)>NOW() THEN '1' ELSE '0' END) FROM tblFeedback WHERE roundId=t2.id ) AS fedbackTime2,
        (CASE WHEN accept IS NOT NULL THEN 'Hire' WHEN reject IS NOT NULL THEN 'Reject'  ELSE 'Pending' END )AS finalStatus")
            ->join('tblRound as t2', 't1.id=t2.resumeId ')
            ->where('t1.deleted', '1')
            ->where('t2.deleted', '1')
        ...
        foreach ($records as $record) {
            $no++;
            if ($record->fedbackTime2 === "1") {
                $startInterView = '<button class="delete fa fa-edit" onClick="startInterview(' . $record->feedbackId . ');"></button>';
            } else {
                $startInterView = 'Not Editable';
            }

            $name = '<a href="' . base_url() . 'Interview/viewResumeDetails/' . $record->id . '/' . $record->roundId . '" >' . $record->name . '</button></a>';
    ...
    {
        $sql = "SELECT * FROM tblFeedback  WHERE id = '$id' AND deleted = '1'";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        // echo $this->db->last_query();
        return $result;
    ...
    {
        $sql = "SELECT  COUNT(id) AS totalInterview ,DATE(createdDate) AS createdDate FROM `tblFeedback` WHERE createdBy='" . $this->session->userdata('userId') . "' GROUP BY DATE(createdDate) ";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        // echo $this->db->last_query();
        // die;
    ...
    {
        $sql = "SELECT COUNT(id) AS total FROM `tblFeedback` WHERE createdBy='" . $this->session->userdata('userId') . "'";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        // echo $this->db->last_query();
    ...
    {
        $sql = "SELECT COUNT(id) AS total, CONCAT(interviewDate,'  ',interviewTime) AS interviewDate FROM `tblRound`  WHERE interviewTaken='0' and interviewerId='" . $this->session->userdata('userId') . "' GROUP BY interviewDate,interviewTime";
        $query = $this->db->query($sql);
        $result = $query->result_array();
    ...
    {
        $sql = "SELECT COUNT(id) AS total FROM tblRound WHERE interviewTaken='0' and interviewerId='" . $this->session->userdata('userId') . "'";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        // echo $this->db->last_query();
        return $result[0]['total'];
    ...
    {
        $sql = "SELECT COUNT(id) AS total ,DATE(createdDate) AS createdDate FROM `tblFeedback` WHERE recommendation='1' and createdBy='" . $this->session->userdata('userId') . "' GROUP BY DATE(createdDate) ";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        // echo $this->db->last_query();
        return $result;
    ...
    {
        $sql = "SELECT COUNT(id) AS total  FROM `tblFeedback` WHERE recommendation='1' and createdBy='" . $this->session->userdata('userId') . "'";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        // echo $this->db->last_query();
        return $result[0]['total'];
    ...
    {
        $sql = "SELECT COUNT(id) AS total ,DATE(createdDate) AS createdDate FROM `tblFeedback` WHERE recommendation='2' and createdBy='" . $this->session->userdata('userId') . "' GROUP BY DATE(createdDate) ";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        // echo $this->db->last_query();
        return $result;
    ...
    {
        $sql = "SELECT COUNT(id) AS total FROM `tblFeedback` WHERE recommendation='2' and createdBy='" . $this->session->userdata('userId') . "' ";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        // echo $this->db->last_query();
        return $result[0]['total'];
    ...
    {
        $sql = "SELECT COUNT(id) AS total ,DATE(createdDate) AS createdDate FROM `tblFeedback` WHERE recommendation='4' and createdBy='" . $this->session->userdata('userId') . "' GROUP BY DATE(createdDate) ";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        // echo $this->db->last_query();
        return $result;
    ...
    {
        $sql = "SELECT COUNT(id) AS total  FROM `tblFeedback` WHERE recommendation='4' and createdBy='" . $this->session->userdata('userId') . "'";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        // echo $this->db->last_query();
        return $result[0]['total'];
    ...
    {
        $sql = "SELECT COUNT(id) AS total ,DATE(createdDate) AS createdDate FROM `tblFeedback` WHERE recommendation='3' and createdBy='" . $this->session->userdata('userId') . "' GROUP BY DATE(createdDate) ";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        // echo $this->db->last_query();
        return $result;
    ...
    {
        $sql = "SELECT COUNT(id) AS total FROM `tblFeedback` WHERE recommendation='3' and createdBy='" . $this->session->userdata('userId') . "' ";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        // echo $this->db->last_query();
        return $result[0]['total'];
